...
include-before: |
    ...
    Showcase different variations of asynchronous API with examples
...
It's a blocking call to GET `/file1.txt`, then to GET `/file2.txt`.
Everything executes synchronously, sequentially. Local variables could
be used to store results.
...
`void*` user data to access later and, finally, run a loop
to give libcurl a chance to process requests. Note, however,
requests execute concurrently now.
