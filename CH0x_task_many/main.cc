...
template<typename T = Task_RefCount>
...
struct WhenAll_State : Task_Storage<typename Task_::type>
{
    Task_ _task;
    WhenAll_State() noexcept
    {
    ...
    void start(Task<T>&& task)
    {
        this->add_ref();
        task.then([this, self = Task_Ptr<>{this}](T v)
        {
            std::get<I>(data()) = std::move(v);
            (void)self; // to decrement ref-count on state, when task is discarded.
        });
    }

    template<typename... Ts, auto... Is>
    void start_all(std::tuple<Task<Ts>...>&& tasks, std::index_sequence<Is...>)
    {
        (this->template start<Is>(std::move(std::get<Is>(tasks))), ...);
    }
};

template<typename T>
using is_value_type = std::bool_constant<!(std::is_same_v<void, T> || std::is_reference_v<T>)>;
